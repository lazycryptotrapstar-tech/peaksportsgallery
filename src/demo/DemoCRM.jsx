...
/* ─── Status helpers ──────────────────────────────────────────────────────── */
const statusAccent = s => s==='hot'?DS.red:s==='warm'?DS.amber:DS.text3
const statusBg     = s => s==='hot'?`${DS.red}15`:s==='warm'?`${DS.amber}15`:`${DS.text3}10`
  ...
  return(
    <div style={{display:'flex',alignItems:'center',marginBottom:14}}>
      {STAGES.map((stage,i)=>{
        ...
        const col=stage==='hot'?DS.red:stage==='warm'?DS.amber:stage==='vip'?DS.gold:DS.text3
        return(
          <React.Fragment key={stage}>
            <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4,zIndex:1}}>
              <div style={{
                ...
                background:isActive||isPast?col:DS.borderLight,
                border:isActive?`2.5px solid ${col}`:isPast?`1.5px solid ${col}`:'none',
                boxShadow:isActive?`0 0 0 4px ${col}25`:'none',
                transition:'all 0.2s',
              }}/>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8,color:isActive?col:isPast?col:DS.text3,textTransform:'uppercase',letterSpacing:'0.06em',fontWeight:isActive?700:400}}>
                {stage}
              </div>
            </div>
            {i<STAGES.length-1&&(
              <div style={{flex:1,height:2,borderRadius:1,background:isPast?col:DS.borderLight,margin:'0 2px 14px',transition:'background 0.2s'}}/>
            )}
          </React.Fragment>
        )
      })}
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:DS.bg}}>
      <style>{`
        ...
        .crm-contact-row:hover .crm-card-bg{background:${DS.bg2} !important}
        @media(max-width:768px){
          .crm-split{flex-direction:column !important}
          .crm-left{width:100% !important;border-right:none !important;border-bottom:1px solid ${DS.borderLight} !important}
      ...
      <div style={{padding:'18px 20px 0',flexShrink:0}}>
        <div className="crm-header-row" style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginBottom:14}}>
          <div>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:DS.text3,letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:3}}>CRM · Midland</div>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:24,fontWeight:800,color:DS.text,letterSpacing:'-0.025em',lineHeight:1.1}}>Sales Pipeline</div>
          </div>
          <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
            {CAMPAIGNS.map(c=>(
              <button key={c.id} onClick={()=>{setCampaign(c.id);setContact(null);setParsed(null)}}
                style={{padding:'6px 14px',borderRadius:20,border:`1.5px solid ${campaign===c.id?DS.gold:DS.border}`,background:campaign===c.id?DS.gold:'transparent',color:campaign===c.id?'white':DS.text2,fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:12,cursor:'pointer',transition:'all 0.15s'}}>
                {c.short}
              </button>
            ))}
          </div>
        ...
        <div className="crm-stats" style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:8,marginBottom:14}}>
          {[
            {label:'Total',  num:stats.total, accent:DS.gold},
            {label:'Hot',    num:stats.hot,   accent:DS.red},
            ...
            <div key={i} style={{...CARD,padding:'10px 14px',position:'relative',overflow:'hidden'}}>
              ...
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:DS.text3,letterSpacing:'0.08em',textTransform:'uppercase',marginBottom:3}}>{s.label}</div>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:22,fontWeight:700,color:DS.text}}>{s.num}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className={`crm-left ${mobileView==='draft'?'mobile-contacts-hide':''}`}
          ...
          <div style={{padding:'0 14px 10px',flexShrink:0}}>
            ...
            <div style={{position:'relative',marginBottom:10}}>
              <svg style={{position:'absolute',left:11,top:'50%',transform:'translateY(-50%)',width:13,height:13,color:DS.text3,pointerEvents:'none'}} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
              <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search contacts…"
                style={{width:'100%',height:36,background:DS.card,border:`1px solid ${DS.border}`,borderRadius:9,padding:'0 12px 0 33px',fontFamily:"'DM Sans',sans-serif",fontSize:13,color:DS.text,outline:'none',boxSizing:'border-box'}}
            ...
            <div style={{display:'flex',gap:5,overflowX:'auto',paddingBottom:2,marginBottom:10,scrollbarWidth:'none'}}>
              {[
                ...
                <button key={f.key} onClick={()=>setStatusFilter(f.key)} style={{
                  ...
                  border:`1.5px solid ${statusFilter===f.key?DS.gold:DS.border}`,
                  background:statusFilter===f.key?DS.goldBg:'transparent',
                  color:statusFilter===f.key?DS.gold:DS.text3,
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
              <div style={{display:'flex',alignItems:'center',gap:5}}>
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9.5,color:DS.text3,letterSpacing:'0.08em',textTransform:'uppercase'}}>Touch</span>
                {[1,2,3].map(t=>(
                  <button key={t} onClick={()=>setTouch(t)} style={{
                    width:28,height:28,borderRadius:7,
                    border:`1.5px solid ${touch===t?DS.gold:DS.border}`,
                    background:touch===t?DS.gold:DS.card,
                    color:touch===t?'white':DS.text2,
                    fontFamily:"'JetBrains Mono',monospace",fontWeight:700,fontSize:11,
                    cursor:'pointer',transition:'all 0.13s',
                  }}>{t}</button>
                ))}
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:DS.text3}}>
                  {touch===1?'Moment':touch===2?'Identity':'Door'}
                </span>
              </div>
              <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{
                background:DS.card,border:`1px solid ${DS.border}`,borderRadius:7,
                padding:'4px 8px',color:DS.text2,fontSize:11,
          ...
          <div style={{flex:1,overflowY:'auto',padding:'0 14px 14px'}}>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:DS.text3,letterSpacing:'0.08em',textTransform:'uppercase',padding:'4px 0 8px'}}>
              ...
              {contacts.map(ct=>{
                ...
                return(
                  <div key={ct.id} className="crm-contact-row"
                    ...
                    style={{borderRadius:11,overflow:'hidden',border:`1px solid ${active?DS.gold:DS.borderLight}`,boxShadow:active?`0 0 0 2px ${DS.gold}25`:'none',transition:'all 0.13s'}}>
                    <div className="crm-card-bg" style={{display:'flex',alignItems:'stretch',background:active?DS.goldPale:DS.card,padding:'11px 11px 11px 0',gap:10,transition:'background 0.13s'}}>
                      {/* Urgency bar */}
                      <div style={{width:4,flexShrink:0,background:accent,borderRadius:'0 2px 2px 0',opacity:active?1:0.7}}/>
                      {/* Avatar */}
                      <div style={{
                        width:36,height:36,borderRadius:9,flexShrink:0,
                        background:active?DS.gold:ct.status==='hot'?`${DS.red}20`:ct.status==='warm'?`${DS.amber}18`:DS.goldPale,
                        color:active?'white':ct.status==='hot'?DS.red:ct.status==='warm'?DS.amber:DS.gold,
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:2}}>
                          <div style={{fontSize:13,fontWeight:700,color:DS.text,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{ct.name}</div>
                          {ct.status==='hot'&&<span style={{fontSize:10,animation:'pulse 2s infinite',flexShrink:0}}>🔥</span>}
                        </div>
                        <div style={{fontSize:11,color:DS.text3,marginBottom:5,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>
                          {ct.title||ct.email}
                        </div>
                        {/* Chips */}
                        <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
                          {(ct.purchase_count||0)>0&&(
                            <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:DS.text3,background:DS.bg,border:`1px solid ${DS.borderLight}`,borderRadius:4,padding:'1px 6px'}}>
                      ...
                      <div style={{display:'flex',flexDirection:'column',alignItems:'flex-end',justifyContent:'space-between',flexShrink:0,gap:6}}>
                        ...
                        <div style={{width:36,height:3,borderRadius:2,background:DS.borderLight,overflow:'hidden'}}>
        ...
        <div className={`crm-right ${mobileView==='contacts'&&!selectedContact?'mobile-draft-hide':''}`}
          ...
          {!selectedContact&&(
            <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:40,textAlign:'center'}}>
              <div style={{width:56,height:56,borderRadius:16,background:DS.goldPale,border:`1px solid ${DS.goldBorder}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:26,marginBottom:14}}>📧</div>
              <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:18,color:DS.text,marginBottom:6}}>Select a Contact</div>
              <div style={{fontSize:13,color:DS.text3,maxWidth:260,lineHeight:1.6}}>Tap any contact to generate an AI-personalized email draft instantly.</div>
            </div>
          )}
          ...
          {selectedContact&&loading&&(
            <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:40,textAlign:'center'}}>
              <div style={{width:56,height:56,borderRadius:'50%',background:DS.goldPale,border:`1.5px solid ${DS.goldBorder}`,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px',fontSize:24}}>🐱</div>
          ...
          {selectedContact&&!loading&&parsed&&(
            <div style={{flex:1,overflowY:'auto',padding:'16px 18px 32px'}}>
              ...
              <div style={{...CARD,marginBottom:14,overflow:'hidden'}}>
                {/* Record header */}
                <div style={{padding:'14px 16px',background:DS.bg,borderBottom:`1px solid ${DS.borderLight}`,display:'flex',alignItems:'center',gap:12}}>
                  <div style={{
                    width:48,height:48,borderRadius:12,flexShrink:0,
                    background:DS.gold,color:'white',
                    display:'flex',alignItems:'center',justifyContent:'center',
                    fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:16,
                  }}>{initials(selectedContact)}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:16,color:DS.text,marginBottom:2}}>{selectedContact.name}</div>
                    <div style={{fontSize:11,color:DS.text3,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{selectedContact.title||selectedContact.email}</div>
                  </div>
                  {/* Status badge */}
                  <div style={{
                ...
                <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)'}}>
                  {[
                    ...
                    <div key={i} style={{padding:'10px 12px',borderRight:i<3?`1px solid ${DS.borderLight}`:'none'}}>
                      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8.5,color:DS.text3,textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:4}}>{item.label}</div>
                      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:15,fontWeight:700,color:item.color||DS.text,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{item.value}</div>
                    </div>
                  ))}
                </div>
                {/* Pipeline bar */}
                <div style={{padding:'12px 16px 8px',borderTop:`1px solid ${DS.borderLight}`}}>
                  <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:DS.text3,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:10}}>Pipeline Stage</div>
                ...
                {[1,2,3].map(t=>(
                  <button key={t} onClick={()=>{setTouch(t);requestDraft(selectedContact,t)}}
                    style={{
                      ...
                      border:`1.5px solid ${touch===t?DS.gold:DS.border}`,
                      background:touch===t?DS.goldBg:DS.card,
                      cursor:'pointer',transition:'all 0.13s',textAlign:'center',
                    }}>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontWeight:700,fontSize:13,color:touch===t?DS.gold:DS.text2,marginBottom:2}}>T{t}</div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:8.5,color:touch===t?DS.gold:DS.text3,letterSpacing:'0.04em'}}>
              ...
              {/* ── Angle banner ────────────────────────────────────────── */}
              {parsed.angle&&(
                <div style={{display:'flex',alignItems:'flex-start',gap:10,padding:'11px 14px',borderRadius:10,background:DS.goldBg,border:`1px solid ${DS.goldBorder}`,marginBottom:14}}>
                  ...
                  <div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:9.5,fontWeight:700,color:DS.gold,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:3}}>{parsed.angle}</div>
                    <div style={{fontSize:12,color:DS.text2,lineHeight:1.6}}>{parsed.reason}</div>
                  </div>
                </div>
              )}
              ...
              <div style={{...CARD,marginBottom:12,overflow:'hidden'}}>
                ...
                <div style={{padding:'10px 14px',borderBottom:`1px solid ${DS.borderLight}`,display:'flex',alignItems:'center',gap:8,background:DS.bg,flexWrap:'wrap'}}>
                  <button onClick={()=>requestDraft(selectedContact,touch)} style={{display:'inline-flex',alignItems:'center',gap:5,padding:'5px 12px',borderRadius:7,border:`1px solid ${DS.border}`,background:DS.card,color:DS.text2,fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:12,cursor:'pointer',transition:'opacity 0.13s'}}
                    onMouseEnter={e=>e.currentTarget.style.opacity='0.75'} onMouseLeave={e=>e.currentTarget.style.opacity='1'}>
                    ↺ Regenerate
                  </button>
                  <button onClick={()=>setEditMode(!editMode)} style={{display:'inline-flex',alignItems:'center',gap:5,padding:'5px 12px',borderRadius:7,border:`1px solid ${editMode?DS.gold:DS.border}`,background:editMode?DS.gold:'transparent',color:editMode?'white':DS.text2,fontFamily:"'DM Sans',sans-serif",fontWeight:600,fontSize:12,cursor:'pointer',transition:'all 0.13s'}}>
                ...
                <div style={{padding:'12px 14px',borderBottom:`1px solid ${DS.borderLight}`}}>
                  ...
                  {editMode
                    ...
                    :<div style={{fontSize:14,fontWeight:600,color:DS.text,lineHeight:1.4}}>{parsed.subject}</div>
                  }
                </div>
                {/* Body */}
                <div style={{padding:'12px 14px'}}>
                  ...
                  {editMode
                    ...
                    :<div style={{fontSize:13,lineHeight:1.85,color:DS.text,whiteSpace:'pre-wrap'}}>{parsed.body}</div>
                  }
                </div>
              </div>
              ...
              {/* Rep note */}
              {parsed.followUp&&(
                <div style={{padding:'11px 14px',borderRadius:10,background:DS.greenBg,border:`1px solid rgba(10,110,56,0.2)`,marginBottom:14}}>
                  <div style={{...LABEL,color:DS.green}}>Rep Note</div>
                  <div style={{fontSize:12,color:'#0a4024',lineHeight:1.65}}>{parsed.followUp}</div>
                </div>
              )}

              {/* Send */}
              <button onClick={openInEmail} style={{width:'100%',padding:'14px',borderRadius:11,border:'none',background:'#16110C',color:DS.gold,fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:15,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',gap:8,transition:'opacity 0.15s'}}
