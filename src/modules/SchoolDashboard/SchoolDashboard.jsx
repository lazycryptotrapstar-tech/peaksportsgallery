  ...
  return (
    <div style={{ flex: 1, minWidth: 0, padding: '12px 14px', background: T.surface, borderRadius: 10, border: `1px solid ${T.border}` }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{label}</span>
        {Icon && <Icon size={12} color={color || T.muted} />}
      </div>
      <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 22, fontWeight: 700, color: color || T.text, lineHeight: 1 }}>{value ?? '—'}</div>
      {sub && <div style={{ fontSize: 10, color: T.muted, marginTop: 3 }}>{sub}</div>}
    </div>
  )
}

function SchoolCard({ school, stats, loading, selected, onSelect }) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: 14,
        border: `1px solid ${isActive ? '#16a34a' : T.border}`,
        boxShadow: isActive ? T.shMd : T.sh,
        overflow: 'hidden',
        cursor: 'pointer',
    ...
    >
      ...
      <div style={{ padding: '16px 18px 14px', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0 }}>
          {/* Logo or emoji */}
          <div style={{ width: 44, height: 44, borderRadius: 10, background: '#F1F5F9', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0, overflow: 'hidden' }}>
            {school.logo_url
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: T.muted, marginTop: 2 }}>{school.conference} · {school.location}</div>
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6, flexShrink: 0 }}>
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, fontWeight: 700, padding: '3px 8px', borderRadius: 4, textTransform: 'uppercase', letterSpacing: '0.06em', background: hasActivity ? T.greenBg : T.amberBg, color: statusColor }}>
      ...
      <div style={{ display: 'flex', gap: 8, padding: '0 18px 14px' }}>
        <KpiTile label="Drafts"         value={loading ? '...' : stats?.drafts}         icon={Zap}        color="#7c3aed" />
        <KpiTile label="Touches Sent"   value={loading ? '...' : stats?.touches}        icon={Mail}       color={T.green} />
        <KpiTile label="Contacts Worked" value={loading ? '...' : stats?.contacted}     icon={Users}      color="#0891b2" />
      </div>
      ...
      <div style={{ padding: '10px 18px 14px', borderTop: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 26, height: 26, borderRadius: 7, background: '#F1F5F9', border: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 11, color: T.text2, flexShrink: 0 }}>
            {stats?.repName?.[0]?.toUpperCase() || '?'}
          </div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: T.text, lineHeight: 1.2 }}>{stats?.repName || 'No rep assigned'}</div>
      ...
      {/* Expanded detail */}
      {isActive && (
        <div style={{ padding: '14px 18px 18px', borderTop: `1px solid ${T.border}`, background: '#F8FAFC' }}>
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 10 }}>Touch Breakdown</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
            <KpiTile label="Touch 1" value={loading ? '...' : stats?.t1} color={T.green} />
            <KpiTile label="Touch 2" value={loading ? '...' : stats?.t2} color={T.amber} />
            <KpiTile label="Touch 3" value={loading ? '...' : stats?.t3} color={T.red}   />
          </div>

          {/* Contacts loaded */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
            <KpiTile label="Contacts Loaded"  value={loading ? '...' : stats?.contactsTotal} icon={Users} color="#0891b2" />
            <KpiTile label="Contacts Added"   value={loading ? '...' : stats?.contactsAdded} icon={Plus}  color={T.green} />
          </div>

          {/* Activity this week */}
          <div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>This Week</div>
...
export default function SchoolDashboard() {
  ...
  // ── Fetch real stats for all schools ───────────────────────────────────
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        // 1. Activity logs for all schools
        const weekAgo = new Date()
        weekAgo.setDate(weekAgo.getDate() - 7)

        const [logsRes, logsWeekRes, contactsRes, repsRes] = await Promise.all([
          supabase.from('activity_log').select('school_id, action, contact_id, user_id, created_at').in('school_id', schoolIds),
          supabase.from('activity_log').select('school_id, action').in('school_id', schoolIds).gte('created_at', weekAgo.toISOString()),
          supabase.from('contacts').select('school_id, contact_type').in('school_id', schoolIds),
        ...
        schools.forEach(school => {
          ...
          const sCts  = contacts.filter(c => c.school_id === sid)
  ...
  const getStats = (school) => statsMap[school.id] || {}
  ...
  const totals = {
    ...
    contacts: Object.values(statsMap).reduce((a, s) => a + (s.contactsTotal || 0), 0),
  }
  ...
  if (loading) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', background: T.bg, flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', gap: 6 }}>
        {[0,1,2].map(i => <div key={i} style={{ width: 8, height: 8, borderRadius: '50%', background: '#16a34a', animation: `bounce 1s ${i*0.15}s infinite` }} />)}
      </div>
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', WebkitOverflowScrolling: 'touch', background: T.bg }}>
      ...
      `}</style>
      <div style={{ padding: '28px 24px 80px', maxWidth: 1000, margin: '0 auto', fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 20 }}>
          {[
            { label: 'Total Schools',    value: schools.length,    color: T.text,    icon: Users      },
            ...
            const Icon = s.icon
            return (
              <div key={s.label} style={{ background: T.surface, borderRadius: 12, border: `1px solid ${T.border}`, padding: '14px 16px', boxShadow: T.sh, position: 'relative', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 3, background: s.color, borderRadius: '12px 0 0 12px' }} />
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
          {[
            ...
            <button key={f.key} onClick={() => setFilter(f.key)} style={{
              ...
              border: `1.5px solid ${filter === f.key ? '#16a34a' : T.border}`,
              background: filter === f.key ? '#dcfce7' : 'transparent',
              color: filter === f.key ? '#16a34a' : T.muted,
        ...
        <div style={{ marginTop: 16, padding: '20px', borderRadius: 14, border: `2px dashed ${T.border}`, textAlign: 'center', cursor: 'default' }}>
          <div style={{ fontSize: 18, marginBottom: 6, color: T.muted }}>+</div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 14, color: T.muted }}>Add New School</div>
