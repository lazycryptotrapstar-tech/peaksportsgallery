...
// ── Scoring ────────────────────────────────────────────────────────────────────
// Compute a 0–100 score from contact data since Vivenue doesn't provide one
const computeScore = (ct) => {
  let s = 30
  s += Math.min(ct.purchase_count * 10, 40)          // up to 40pts for loyalty
...
const CSS = (primary) => `
  ...
  .pb-sport-pill { padding:6px 16px; border-radius:20px; border:1px solid var(--pb-border); background:var(--pb-surface2); color:var(--pb-muted); cursor:pointer; font-family:'Space Mono',monospace; font-size:10px; font-weight:700; letter-spacing:0.08em; text-transform:uppercase; transition:all 0.15s; }
...
export default function CRM() {
  ...
  const [sportFilter, setSportFilter]     = useState('All')  // 'All' | 'Football' | 'Volleyball'

  const allContacts = campaign ? getContacts(school.id, campaign) : []

  // ── Sport filter (only meaningful for Wofford tickets) ──────────────────────
  const contacts = useMemo(() => {
    if (campaign !== 'TICKETS' || sportFilter === 'All') return allContacts
    return allContacts.filter(ct => {
      const sport = ct.sport || ''
      if (sportFilter === 'Football')   return sport.includes('Football')
      if (sportFilter === 'Volleyball') return sport.includes('Volleyball')
      return true
    })
  }, [allContacts, sportFilter, campaign])

  // ── Dynamic stats ────────────────────────────────────────────────────────────
  const stats = useMemo(() => {
    const all = getContacts(school.id, 'TICKETS')
    const hot  = all.filter(ct => ct.status === 'hot').length
    const cold = all.filter(ct => ct.status === 'cold').length
    return { total: all.length, hot, lapsed: cold }
  }, [school.id])

  // ── Sport counts for filter pills ────────────────────────────────────────────
  const sportCounts = useMemo(() => {
    if (campaign !== 'TICKETS') return {}
    const all = getContacts(school.id, 'TICKETS')
    return {
      All:        all.length,
      Football:   all.filter(ct => ct.sport?.includes('Football')).length,
      Volleyball: all.filter(ct => ct.sport?.includes('Volleyball')).length,
    }
  }, [school.id, campaign])
      ...
      const res = await fetch(PLAYBOOK_WEBHOOK, {
        ...
        body: JSON.stringify({
          ...
          contact: {
            ...
            // ── New fields the AI can now use ──
            status:         ct.status || 'warm',       // hot / warm / cold
            last_year:      ct.last_year || 0,         // e.g. 2024
            sport:          ct.sport || 'Football',    // Football / Volleyball / Football+Volleyball
            tags:           ct.tags || '',             // e.g. "FB23,FB24,VLOT22"
            city:           ct.city || '',
  ...
  const reset = () => {
    ...
    setTouch(1); setParsed(null); setEditMode(false); setSportFilter('All')
  }
        ...
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '24px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
            {[
              ...
              { label: 'Lapsed Buyers',  value: stats.lapsed },
            ].map(s => (
      ...
      {stage === 'contacts' && (
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '20px 24px' }}>
          ...
          <p style={{ fontFamily: "'Space Mono',monospace", fontSize: 10, color: 'var(--pb-muted)', marginBottom: 12 }}>
            {contacts.length} contacts · Touch {touch} · {touch === 1 ? 'The Moment' : touch === 2 ? 'The Identity' : 'The Door'}
          </p>
            ...
            {contacts.map(ct => {
              ...
              return (
                <button key={ct.id} className="pb-contact-row" onClick={() => { setContact(ct); requestDraft(ct, touch) }}>
                  ...
                  {/* Score */}
                  <div className="pb-score" style={{ background: SCORE_COLOR(score), flexShrink: 0 }}>
                    {score}
                  </div>
                  {/* Status badge */}
