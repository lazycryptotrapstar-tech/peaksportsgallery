...
// Venue section data per sport type
...
const ZONE_LABELS = {
  home_chairback: 'Home Chairback',
  home_bleacher: 'Home Bleacher',
  vip: 'VIP / Hospitality',
  end_zone: 'End Zone',
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: school.colors.accent, marginBottom: 6 }}>Wofford Terriers · Ticket Marketplace</p>
              ...
              <div>
                    ...
                    {[1,2,3,4,5,6].map(n => (
                      <button key={n} onClick={() => setQty(n)}
                        style={{
                          ...
                          background: qty === n ? school.colors.primary : '#f1f5f9',
                          color: qty === n ? 'white' : '#64748b',
                ...
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 14, marginBottom: 16 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4, fontSize: 12, color: '#94a3b8' }}>
                    <span>Facility fee ({qty}x)</span>
                    <span>${(2.50 * qty).toFixed(2)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#94a3b8' }}>
                    <span>Processing ({qty}x)</span>
                    <span>${(1.50 * qty).toFixed(2)}</span>
                  </div>
                ...
                <button onClick={() => setStage('success')}
                  style={{
                    ...
                    background: school.colors.primary, color: 'white', cursor: 'pointer',
                    fontFamily: "'Rajdhani', sans-serif", fontWeight: 700, fontSize: 17,
                    marginBottom: 10, transition: 'opacity 0.2s ease',
                  }}>
                  Purchase Tickets →
                </button>
      ...
      <style>{`
        @media (max-width: 768px) {
          .ticketing-grid { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </div>
  )
